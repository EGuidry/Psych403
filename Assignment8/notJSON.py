...
for block in range(nBlocks):
    ...
    #-draw end block text
    endbl_msg = "End of Block"
    #-define stimuli using psychopy functions
    endbl_text = visual.TextStim(win, text = endbl_msg)
    endbl_text.color = 'blueviolet'
    endbl_text.draw()
    #-flip window
    win.flip() 
    #-wait time (stimulus duration)
...
#-draw end trial text
endtr_msg = "End of Trial. Press any key to close."
#-define stimuli using psychopy functions
endtr_text = visual.TextStim(win, text = endtr_msg)
endtr_text.color = 'lawngreen'
endtr_text.draw()
#-flip window
win.flip() 
#-wait time (stimulus duration)
...
# Could not get JSON to work, so I tried with cvs
with open(fullAddress, 'w', newline='') as sub_data: #added "newline=" <-- New stuff for CVS exercise 
...
df=pd.read_csv(fullAddress) #load/reading file
filteredTable = df.loc[(df['sub_acc'] == 1) & (df['sub_resp'] > 0)] #removing inaacurate responses 
print(filteredTable) #resluts
print('resp_time mean:', df['resp_time'][(df['sub_acc'] == 1) & (df['sub_resp'] > 0)].mean()) #getting the mean
